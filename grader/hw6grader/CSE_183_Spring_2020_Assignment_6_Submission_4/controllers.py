...
def get_posts():
    ...
    # TODO: complete.
    postList = []
    posts = db(db.post.is_reply == None).select(orderby=~db.post.post_date).as_list()  # Just to keep code from breaking.
    for p in posts:
        p['author'] = get_name_from_email(p.get('email'))
        replyList = db(db.post.is_reply == p.get('id')).select(orderby=~db.post.post_date).as_list()
        for r in replyList:
            r['author'] = get_name_from_email(r.get('email'))

        postList.append(p)
        postList.extend(replyList)
    return dict(posts=postList)
...
def save_post():
    ...
    # TODO: complete.
    if id is None:
       return_id =  db.post.insert(id=id,content=content, is_reply=is_reply)
    else:
        updatePost = db(db.post.id == id).select().first()
        updatePost.content = content
        updatePost.update_record()
    # If id is None, this means that this is a new post that needs to be
    # inserted.  If id is not None, then this is an update.
    return dict(content=content, id=return_id, is_reply=is_reply)
