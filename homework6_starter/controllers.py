...
def get_posts():
    ...
    # TODO: complete.
    output = []
    posts = db(db.post.is_reply == None).select(orderby=~db.post.id).as_list()
    replies = db(db.post.is_reply != None).select(orderby=~db.post.id).as_list()
    for post in posts:
        output.append(post)
        for reply in replies:
            if(post.get('id') == reply.get('is_reply')):
                output.append(reply)  
    posts = output
    for post in posts:
        user_entry = db(db.auth_user.email == post.get('email')).select().first()
        post['author'] = user_entry.first_name + " " + user_entry.last_name
